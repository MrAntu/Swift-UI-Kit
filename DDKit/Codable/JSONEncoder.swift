...
private struct _JSONKeyedEncodingContainer<K: CodingKey> : KeyedEncodingContainerProtocol {
    ...
    public mutating func nestedContainer<NestedKey>(keyedBy keyType: NestedKey.Type, forKey key: Key) -> KeyedEncodingContainer<NestedKey> {
        ...
        let coder = self.encoder
        let path = self.codingPath

        return self.with(pushedKey: key) {
            let container = _JSONKeyedEncodingContainer<NestedKey>(referencing: coder, codingPath: path, wrapping: dictionary)
            return KeyedEncodingContainer(container)
        }
    }
    ...
    public mutating func nestedUnkeyedContainer(forKey key: Key) -> UnkeyedEncodingContainer {
        ...
        let path = self.codingPath
        return self.with(pushedKey: key) {
            _JSONUnkeyedEncodingContainer(referencing: coder, codingPath: path, wrapping: array)
        }
    }
...
extension _JSONEncoder {
    ...
    fileprivate func box<T: Encodable>(_ value: T) throws -> NSObject {
        if T.self == Date.self {
            ...
            /****************************** 改编译问题 internal，实测常规解析这里没有执行过
             * 替换掉以下两句，因为 doubleValue 为 internal
            let doubleValue = (value as! Decimal).doubleValue
            return try self.box(doubleValue)
            *******************************/
            // https://stackoverflow.com/questions/39890795/decimal-to-double-conversion-in-swift-3
            return try self.box(Double(truncating: (value as! Decimal) as NSNumber))
            /******************************/
        }
...
private class _JSONReferencingEncoder: _JSONEncoder {
...
open class JSONDecoder {
    ...
    open func decode<T: Decodable>(_ type: T.Type, from data: Data) throws -> T {
        /******************************** 改编译问题 internal
         * 修改这一句，不确定是否会引发效率或内存问题，.useReferenceNumericTypes 为 internal 分析 JSONSerialization 源码后，其功能是把数字解析成 NSNumber，而非 Int 和 Double
         * https://github.com/apple/swift-corelibs-foundation/blob/3abd700d1ffab10fceaec665e12e6a0af5cd8870/Foundation/JSONSerialization.swift#L847
...
private class _JSONDecoder: Decoder {
    // MARK: Properties
...
extension _JSONDecoder {
    ...
    fileprivate func unbox(_ value: Any, as type: Bool.Type) throws -> Bool? {
        ...
        /******************************** 改编译问题 internal
         * 替换掉这一段，因为 _cfTypeID 是 internal 成员
    ...
    fileprivate func unbox(_ value: Any, as type: Int.Type) throws -> Int? {
        guard !(value is NSNull) else { return nil }
        /******************************* 功能增强，添加以下代码，兼容 String 类型 */
        if let string = value as? String, let int = Int(string), string == "\(int)" {
            return int
        }
        /******************************/
    ...
    fileprivate func unbox(_ value: Any, as type: Float.Type) throws -> Float? {
        ...
        */
        ...
        /******************************* 功能增强，添加以下代码，兼容 String 类型 */
        else if let string = value as? String, let float = Float(string), string == "\(float)" {
            return float
        }
        /******************************/
